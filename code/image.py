...
class ImageCaptcha(_Captcha):
    ...
    def create_captcha_image(self, chars, color, background):
        ...
        def _draw_character(c):
            ...
            Draw(im).text((dx-1, dy), c, font=font, fill=color)
            Draw(im).text((dx+1, dy), c, font=font, fill=color)
            Draw(im).text((dx, dy-1), c, font=font, fill=color)
            Draw(im).text((dx, dy+1), c, font=font, fill=color)
            ...
            Draw(im).text((dx, dy), c, font=font, fill=background)
        ...
        for im in images:
            ...
            image.paste(im, (offset, int((self._height - h) / 2)-7), mask)
